...
Requirements: local VS Code Desktop 1.101+ on macOS or Linux, and Node.js 22.13+.
...
3. Choose **Codex — install automatically**, review the generated TOML block, and select
   **Install**.
4. Restart Codex.
5. Open the repository you want to use in VS Code, trust it, then run **VS Code MCP:
   Enable for This Workspace**.
6. Ask Codex to call `list_instances`. The enabled VS Code window should appear and all
   read/LSP tools are ready. Enable write or task/debug access separately from the
   Command Palette only when needed.

That is the complete normal installation. No repository clone, Docker container,
package-manager command, separate server download, or hand-written config is required.
Use [the full installation guide](./docs/installation.md) for generic MCP clients,
manual archives, repair, removal, upgrades, and troubleshooting.
...
- Client setup is an explicit UI command. It verifies the version-matched bundled server
  by SHA-256, probes only selected Node executables with fixed `--version`, and mutates
  only its owned Codex TOML block after preview, backup, and race checks.
