...
not create an unreviewed artifact, expose a long-lived publishing secret, imply that the
server is bundled, or weaken the exact contract and security gates.
...
- The Marketplace listing must state that the separately installed matching server is
  required and link to the corresponding GitHub Release installation guide.
...
- Marketplace users receive a signed, discoverable extension but still need the matching
