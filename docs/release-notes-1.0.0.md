...
extension VSIX is also published to the VS Code Marketplace and contains the exact
SHA-256-manifested server for **VS Code MCP: Set Up MCP Client**. The standalone server
archive remains in GitHub Releases for manual installation. Follow the release's
`INSTALLATION.md` guide. Open VSX, npm, JSR, Yarn, and Bun are not distribution
channels.
