...
| F1-SET-001 | Pass   | `client-setup.unit.test.ts`, the static surface audit, and release-verifier tamper tests prove marker-owned Codex updates, preview races, symlink refusal, exclusive backups, fixed Node probing, bundled-server version/SHA verification, repair, and removal.            |
...
version drift, bundled-setup-server tampering, disconnected CycloneDX components,
missing/extra checksums, unsafe paths, and noncanonical manifests. Artifact tests delete
their VS Code caches before and after each run.
...
The exact candidate payload digests are generated from the reviewed source snapshot and
recorded in `artifacts/release-1.0.0/SHA256SUMS`; mutable draft documentation does not
embed stale artifact hashes.
