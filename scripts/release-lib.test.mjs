...
describe('release version preflight', () => {
  it('rejects a mismatched workspace package before packaging', async () => {
    ...
    for (const [relativePath, version] of [
      ['package.json', '1.1.1'],
      ['packages/extension/package.json', '1.1.1'],
      ['packages/server/package.json', '0.9.0'],
      ['packages/protocol/package.json', '1.1.1'],
    ]) {
    ...
    await expect(assertReleaseVersions(root)).rejects.toThrow(
      'packages/server/package.json: expected 1.1.1, found 0.9.0',
    );
...
describe('deterministic release archives', () => {
  ...
  it('stages the exact deterministic standalone CLI archive contents', async () => {
    ...
    expect(await readdir(destinationDirectory)).toEqual([
      'vscode-mcp-server-1.1.1.tar.gz',
    ]);
  });
});
...
describe('release metadata', () => {
  ...
  it('rejects version drift inside checksummed VSIX and server archives', async () => {
    ...
    const vsixPath = path.join(root, 'vscode-mcp-extension-1.1.1.vsix');
    ...
    await writeValidReleaseDirectory(root);
    const serverPath = path.join(root, 'vscode-mcp-server-1.1.1.tar.gz');
  ...
  it('rejects a tampered bundled setup server inside a checksummed VSIX', async () => {
    ...
    await expect(verifyReleaseDirectory(root)).rejects.toThrow(
      'VSIX bundled server does not match the release contract.',
    );
  });
  ...
  it('reads active VSIX XML structure and ignores metadata hidden in comments', async () => {
    ...
    manifest.contents = Buffer.from(
      source.replace(
        '<Identity Language="en-US" Id="vscode-mcp" Version="1.1.1" Publisher="vscode-mcp"/>',
        '<!-- <Identity Language="en-US" Id="vscode-mcp" Version="1.1.1" Publisher="vscode-mcp"/> --><Identity Language="en-US" Id="vscode-mcp" Version="1.1.1" Publisher="attacker"/>',
      ),
    );
    await replaceReleaseArtifact(root, vsixPath, buildDeterministicZip(entries));
  ...
  it('rejects a checksummed SBOM whose dependency graph is incomplete', async () => {
    const root = await temporaryDirectory();
    await writeValidReleaseDirectory(root);
    const sbomFileName = 'vscode-mcp-1.1.1.cdx.json';
  ...
  it('rejects SBOM components disconnected from the release root', async () => {
    ...
    const sbomPath = path.join(root, sbomFileName);
    const sbom = JSON.parse(await readFile(sbomPath, 'utf8'));
    const disconnectedRef = 'pkg:npm/disconnected@1.1.1';
  ...
  it('creates a stable CycloneDX 1.5 BOM without local paths', () => {
    ...
    const graph = {
      ...
      dependencies: new Map([
        ['pkg:generic/vscode-mcp-release@1.1.1', new Set([component.bomRef])],
        [component.bomRef, new Set()],
      ]),
      rootRef: 'pkg:generic/vscode-mcp-release@1.1.1',
      version: '1.1.1',
    };
...
async function writeValidReleaseDirectory(root) {
  ...
  const rootRef = 'pkg:generic/vscode-mcp-release@1.1.1';
  ...
  const vsix = buildDeterministicZip([
    ...
    },
    {
      ...
      mode: 0o644,
      contents: Buffer.from(
        '<PackageManifest><Metadata><Identity Language="en-US" Id="vscode-mcp" Version="1.1.1" Publisher="vscode-mcp"/><Properties><Property Id="Microsoft.VisualStudio.Code.Engine" Value="^1.101.0" /></Properties></Metadata><Assets><Asset Type="Microsoft.VisualStudio.Code.Manifest" Path="extension/package.json" /></Assets></PackageManifest>\n',
      ),
    },
    {
    ...
    ...[
      ...
      ['extension/dist/extension.js', '(()=>{"vscode-mcp":"1.1.1"})();\n'],
  ...
  const specifications = [
    ['vscode-mcp-extension-1.1.1.vsix', 'extension', 'application/vsix', vsix],
    ['vscode-mcp-server-1.1.1.tar.gz', 'server', 'application/gzip', server],
    ['vscode-mcp-1.1.1.cdx.json', 'sbom', 'application/vnd.cyclonedx+json', sbom],
